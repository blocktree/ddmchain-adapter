...
func (singer *TransactionSigner) SignTransactionHash(msg []byte, privateKey []byte, eccType uint32) ([]byte, error) {
	sig, err := DdmSignature(privateKey, msg)
...
func ddm_hmac_rfc6979_init(key []byte, keylen int) ([]byte, int) {
	k := make([]byte, 32)
	v := make([]byte, 32)
	out := make([]byte, 64)
...
*/
func ddm_hmac_rfc6979_gnerate(k, v []byte, retry, nouncelen int) ([]byte, int) {
...
func ddm_nonce_function_rfc6979(msg, key, algo, extradata []byte, counter uint32) []byte {
	...
	ret, retry := ddm_hmac_rfc6979_init(keydata, keylen)

	for i := uint32(0); i <= counter; i++ {
		ReTry := retry
		temp, retry := ddm_hmac_rfc6979_gnerate(ret[:32], ret[32:], ReTry, 32)
...
func ddm_set_uint64(a []byte) []uint64 {
...
func ddm_set_uint32(a []byte) []uint32 {
...
func ddm_check_overflow_uint64(a []byte) bool {
	var yes bool
	var no bool
	yes = false
	no = false
	curveOrder := owcrypt.GetCurveOrder(owcrypt.ECC_CURVE_SECP256K1)
	a_uint64 := ddm_set_uint64(a)
	curveOrder_uint64 := ddm_set_uint64(curveOrder)
...
func ddm_check_overflow_uint32(a []byte) bool {
	var yes bool
	var no bool
	yes = false
	no = false
	curveOrder := owcrypt.GetCurveOrder(owcrypt.ECC_CURVE_SECP256K1)
	a_uint32 := ddm_set_uint32(a)
	curveOrder_uint32 := ddm_set_uint32(curveOrder)
...
func ddm_check_is_zero(a []byte) bool {
	b := make([]uint64, len(a)>>3)
	b = ddm_set_uint64(a)
	return ((b[3] == 0) && (b[2] == 0) && (b[1] == 0) && (b[0] == 0))
}

func ddm_signatureInner(prikey []byte, hash []byte, nounce []byte) ([]byte, uint16) {
	var recid byte
...
the second part
*/
func DdmSignature(prikey []byte, hash []byte) ([]byte, uint16) {
	signature := make([]byte, 65)
	//	var recid byte
	var ret uint16
	var counter uint32
	...
	prikey_overflow := ddm_check_overflow_uint64(prikey)
	prikey_IsZero := ddm_check_is_zero(prikey)
	if !prikey_overflow && !prikey_IsZero {
		for {
			nounce := ddm_nonce_function_rfc6979(hash, prikey, nil, nil, counter)
			nounce_overflow := ddm_check_overflow_uint32(nounce)
			nounce_IsZero := ddm_check_is_zero(nounce)
			if !nounce_overflow && !nounce_IsZero {
				signature, ret = ddm_signatureInner(prikey, hash, nounce)
				if ret == owcrypt.SUCCESS {
					break
				}
			}
