...
func NewDDMBlockScanner(wm *WalletManager) *DDMBLockScanner {
	bs := DDMBLockScanner{
...
func (this *DDMBLockScanner) SetRescanBlockHeight(height uint64) error {
	height = height - 1
	if height < 0 {
		return errors.New("block height to rescan must greater than 0.")
	}

	block, err := this.wm.WalletClient.ddmGetBlockSpecByBlockNum(height, false)
	if err != nil {
...
func (this *DDMBLockScanner) newBlockNotify(block *DdmBlock, isFork bool) {
...
func (this *DDMBLockScanner) ScanBlock(height uint64) error {
	curBlock, err := this.wm.WalletClient.ddmGetBlockSpecByBlockNum(height, true)
	if err != nil {
		this.wm.Log.Errorf("ddmGetBlockSpecByBlockNum failed, err = %v", err)
		return err
	}
...
func (this *DDMBLockScanner) ScanTxMemPool() error {
...
func (this *DDMBLockScanner) RescanFailedTransactions() error {
...
func (this *DDMBLockScanner) ScanBlockTask() {
	...
	for {
		...
		maxBlockHeight, err := this.wm.WalletClient.ddmGetBlockNumber()
		if err != nil {
			this.wm.Log.Errorf("get max height of ddm failed, err=%v", err)
			break
		}
		...
		curBlock, err := this.wm.WalletClient.ddmGetBlockSpecByBlockNum(curBlockHeight, true)
		if err != nil {
			this.wm.Log.Errorf("ddmGetBlockSpecByBlockNum failed, err = %v", err)
			break
		}
		...
		if curBlock.PreviousHash != curBlockHash {
			...
			if err != nil && err != storm.ErrNotFound {
				...
				curBlock, err = this.wm.WalletClient.ddmGetBlockSpecByBlockNum(curBlockHeight, false)
				if err != nil {
					this.wm.Log.Errorf("ddmGetBlockSpecByBlockNum  failed, block number=%v, err=%v", curBlockHeight, err)
					break
				}
			}
...
func (this *DDMBLockScanner) newExtractDataNotify(height uint64, tx *BlockTransaction, extractDataList map[string][]*openwallet.TxExtractData) error {
...
func (this *DDMBLockScanner) BatchExtractTransaction(txs []BlockTransaction) error {
	for i := range txs {
...
func (this *DDMBLockScanner) GetTxPoolPendingTxs() ([]BlockTransaction, error) {
	txpoolContent, err := this.wm.WalletClient.ddmGetTxPoolContent()
	if err != nil {
...
// func (this *DDMBLockScanner) InitEthTokenExtractResult(tx *BlockTransaction, tokenEvent *TransferEvent, result *ExtractResult, isFromAccount bool) {
...
// 	fee, err := ConverWeiStringToDdmDecimal(feeInteger.String())
// 	if err != nil {
// 		this.wm.Log.Errorf("convert to ddm decimal failed, err=%v", err)
...
// func (this *DDMBLockScanner) InitEthExtractResult(tx *BlockTransaction, result *ExtractResult, isFromAccount bool) {
// 	txExtractData := &openwallet.TxExtractData{}
...
// 	amountVal, _ := ConverWeiStringToDdmDecimal(amount.String())
...
func (this *WalletManager) GetErc20TokenEvent(transactionID string) (*TransferEvent, error) {
	receipt, err := this.WalletClient.ddmGetTransactionReceipt(transactionID)
	if err != nil {
...
func (this *DDMBLockScanner) UpdateTxByReceipt(tx *BlockTransaction) (*TransferEvent, error) {
	//过滤掉未打包交易
	if tx.BlockHeight == 0 || tx.BlockHash == "" {
		return nil, nil
	}

	receipt, err := this.wm.WalletClient.ddmGetTransactionReceipt(tx.Hash)
	if err != nil {
...
/*func (this *DDMBLockScanner) GetErc20TokenEvent(tx *BlockTransaction) (*TransferEvent, error) {
	//非合约交易或未打包交易跳过
...
func (this *DDMBLockScanner) MakeToExtractData(tx *BlockTransaction, tokenEvent *TransferEvent) (string, []*openwallet.TxExtractData, error) {
	if tokenEvent == nil {
...
func (this *DDMBLockScanner) MakeSimpleToExtractData(tx *BlockTransaction) (string, []*openwallet.TxExtractData, error) {
	var sourceKey string
	var exist bool
	...
	if err != nil {
		this.wm.Log.Errorf("calc tx fee in ddm failed, err=%v", err)
		return "", extractDataList, err
	}

	amountVal, err := tx.GetAmountEthString()
	if err != nil {
		this.wm.Log.Errorf("calc amount to ddm decimal failed, err=%v", err)
		return "", extractDataList, err
	}
...
func (this *DDMBLockScanner) GetBalanceByAddress(address ...string) ([]*openwallet.Balance, error) {
	...
	query := func(addr *addressBalance) {
		...
		balanceConfirmed, err := this.wm.WalletClient.GetAddrBalance(appendOxToAddress(addr.Address))
		if err != nil {
			this.wm.Log.Error("get address[", addr.Address, "] balance failed, err=", err)
			return
		}

		balanceAll, err := this.wm.WalletClient.GetAddrBalance(appendOxToAddress(addr.Address))
		if err != nil {
		...
		confirmed, err := ConverWeiStringToDdmDecimal(balanceConfirmed.String())
		if err != nil {
			this.wm.Log.Errorf("ConverWeiStringToDdmDecimal confirmed balance failed, err=%v", err)
			return
		}
		all, err := ConverWeiStringToDdmDecimal(balanceAll.String())
		if err != nil {
			this.wm.Log.Errorf("ConverWeiStringToDdmDecimal all balance failed, err=%v", err)
			return
		}

		unconfirmed, err := ConverWeiStringToDdmDecimal(balanceUnconfirmed.String())
		if err != nil {
			this.wm.Log.Errorf("ConverWeiStringToDdmDecimal unconfirmed balance failed, err=%v", err)
			return
		}
...
func (this *DDMBLockScanner) MakeTokenToExtractData(tx *BlockTransaction, tokenEvent *TransferEvent) (string, []*openwallet.TxExtractData, error) {
	var sourceKey string
	var exist bool
	var extractDataList []*openwallet.TxExtractData
...
func (this *DDMBLockScanner) MakeFromExtractData(tx *BlockTransaction, tokenEvent *TransferEvent) (string, []*openwallet.TxExtractData, error) {
	if tokenEvent == nil {
...
func (this *DDMBLockScanner) MakeSimpleTxFromExtractData(tx *BlockTransaction) (string, []*openwallet.TxExtractData, error) {
	var sourceKey string
	var exist bool
	var extractDataList []*openwallet.TxExtractData
...
func (this *DDMBLockScanner) MakeTokenTxFromExtractData(tx *BlockTransaction, tokenEvent *TransferEvent) (string, []*openwallet.TxExtractData, error) {
	var sourceKey string
	var exist bool
	var extractDataList []*openwallet.TxExtractData
...
func (this *DDMBLockScanner) ExtractTransactionData(txid string, scanTargetFunc openwallet.BlockScanTargetFunc) (map[string][]*openwallet.TxExtractData, error) {
	//result := bs.ExtractTransaction(0, "", txid, scanAddressFunc)
	tx, err := this.wm.WalletClient.DdmGetTransactionByHash(txid)
	if err != nil {
...
func (this *DDMBLockScanner) TransactionScanning(tx *BlockTransaction) (*ExtractResult, error) {
...
func (this *DDMBLockScanner) GetScannedBlockHeader() (*openwallet.BlockHeader, error) {
	...
	//如果本地没有记录，查询接口的高度
	if blockHeight == 0 {
		blockHeight, err = this.wm.WalletClient.ddmGetBlockNumber()
		if err != nil {
			this.wm.Log.Errorf("ddmGetBlockNumber failed, err=%v", err)
			return nil, err
		}
		...
		block, err := this.wm.WalletClient.ddmGetBlockSpecByBlockNum(blockHeight, false)
		if err != nil {
...
func (this *DDMBLockScanner) GetCurrentBlockHeader() (*openwallet.BlockHeader, error) {
...
func (this *DDMBLockScanner) GetGlobalMaxBlockHeight() uint64 {
