...
func ConverWeiStringToDdmDecimal(amount string) (decimal.Decimal, error) {
...
func (this *WalletManager) RecoverBlockHeader(height uint64) (*DdmBlock, error) {
	db, err := OpenDB(this.GetConfig().DbPath, this.GetConfig().BlockchainFile)
	if err != nil {
...
func (this *WalletManager) SaveBlockHeader(block *DdmBlock) error {
	db, err := OpenDB(this.GetConfig().DbPath, this.GetConfig().BlockchainFile)
	if err != nil {
		this.Log.Errorf("open db for save block failed, err=%v", err)
		return err
...
func (this *WalletManager) SaveBlockHeader2(block *DdmBlock) error {
	db, err := OpenDB(this.GetConfig().DbPath, this.GetConfig().BlockchainFile)
	if err != nil {
		this.Log.Errorf("open db for save block failed, err=%v", err)
		return err
