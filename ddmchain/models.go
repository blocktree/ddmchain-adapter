...
type DdmTransactionReceipt struct {
	Logs    []DdmEvent `json:"logs"`
...
func (this *DdmTransactionReceipt) ParseTransferEvent() *TransferEvent {
	...
	for i, _ := range this.Logs {
		...
		if this.Logs[i].Topics[0] != DDM_TRANSFER_EVENT_ID {
			continue
		}
...
func (this *BlockTransaction) GetAmountEthString() (string, error) {
	...
	amountVal, err := ConverWeiStringToDdmDecimal(amount.String())
	if err != nil {
		log.Errorf("convert tx.Amount to ddm decimal failed, err=%v", err)
		return "", err
	}
...
func (this *BlockTransaction) GetTxFeeEthString() (string, error) {
	...
	feeprice, err := ConverWeiStringToDdmDecimal(fee.String())
	if err != nil {
