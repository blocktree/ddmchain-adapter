...
type WalletManager struct {
	...
	Blockscanner *DDMBLockScanner              //区块扫描器
...
func NewWalletManager() *WalletManager {
	...
	wm.ContractDecoder = &DdmContractDecoder{
		wm: &wm,
	}
...
func (this *WalletManager) GetERC20TokenTransactionFeeEstimated(from string, to string, data string) (*txFeeInfo, error) {
	/*gasLimit, err := ddmGetGasPrice(makeERC20TokenTransGasEstimatePara(from, to, data))
	if err != nil {
		this.Log.Errorf(fmt.Sprintf("get gas limit failed, err = %v\n", err))
		return nil, err
	}

	gasPrice, err := ddmGetGasPriceNoParam()
	if err != nil {
		this.Log.Errorf(fmt.Sprintf("get gas price failed, err = %v\n", err))
		return nil, err
	}
...
func (this *WalletManager) GetSimpleTransactionFeeEstimated(from string, to string, amount *big.Int) (*txFeeInfo, error) {
	/*gasLimit, err := ddmGetGasPrice(makeSimpleTransGasEstimatedPara(from, to, amount))
	if err != nil {
		this.Log.Errorf(fmt.Sprintf("get gas limit failed, err = %v\n", err))
		return nil, err
	}
...
func (this *Client) ddmGetGasPriceNoParam() (*big.Int, error) {
	params := []interface{}{}
...
func (this *WalletManager) GetNonceForAddress2(address string) (uint64, error) {
	...
	//txpool, err := this.WalletClient.ddmGetTxPoolContent()
	//if err != nil {
	...
	nonce, err := this.WalletClient.GetAddressNonce(address)
	if err != nil {
		log.Error("GetNonceForAddress2 failed, err=", err)
		return 0, err
	}
	log.Debugf("nonce:%v", nonce)
	if nonce == nil{
		return 0,errors.New("GetNonceForAddress2 faild,= nonce is nil")
	}
...
func (this *WalletManager) GetNonceForAddress(address string) (uint64, error) {
	...
	nonce, err := this.WalletClient.ddmGetTransactionCount(address)
	if err != nil {
